...
const cors = require('cors');//책201p 추가

let corsOption = {
    origin: 'http://localhost:8080', //허락하는 요청 주소
    Credential:true //true로 설정하면 설정한 내용을 response 헤더에 추가 해줍니다.
}
...
fs.watchFile(__dirname + '/sql.js', (curr, prev)=>{
    ...
    delete require.cache[require.resolve('./spl.js')];
    sql = require('./sql.js');
});
...
request.body.param[0][key];
            ...
            res.send({
                error: "Please try again or contact system mangager."
            });
        }
    }catch (err){
        res.send({
...
app.post('/upload/:productId/:type/fileName', async (request, res) => {
    ...
    const data = request.body.data.silce(request.body.data.indexOf(';base64,') + 8);
